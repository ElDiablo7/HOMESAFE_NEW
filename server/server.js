...
app.post('/api/brain', rateLimitMiddleware, async (req, res) => {
  ...
  const ollamaModel = selectOllamaModel(messageText);

  log('info', `Brain request from module: ${module || 'unknown'} (Primary: ${primaryProvider})`, {
    requestId: req.requestId,
    primaryProvider,
  ...
  try {
    ...
    const providersToTry = [primaryProvider, 'openai', 'anthropic', 'openrouter', 'ollama'];
    const uniqueProviders = [...new Set(providersToTry.filter(p => !!p))];
    ...
    for (const currentProvider of uniqueProviders) {
      try {
        log('info', `Attempting brain request with: ${currentProvider}`, { requestId: req.requestId });
        ...
        lastError = err;
        log('warn', `Provider ${currentProvider} failed: ${err.message}`, { requestId: req.requestId });
    ...
    log('info', `Brain response generated by ${finalProvider} in ${duration}ms`, { requestId: req.requestId });
    ...
    res.json({
      ...
      failover: finalProvider !== primaryProvider
    });

  } catch (error) {
    log('error', `Brain API terminal error: ${error.message}`, { requestId: req.requestId });

    const statusCode = error.statusCode || 500;
    res.status(statusCode).json({
      error: 'Failed to get AI response after failover attempts',
